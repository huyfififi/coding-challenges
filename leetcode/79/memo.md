...
- [thonda28 さんのPR](https://github.com/thonda28/leetcode/pull/14)
	- どれか 1 ルートが `True`を返してきたら後の結果は使わないので、すぐに`return`するのも確かにあるな。
		- `step2\_skip\_redundant\_recursions.py` で LeetCode 上の実行時間ランキング Beats 100% になった (もちろん、LeetCode上の実行時間・メモリ使用量はそこまで信頼できるものではないことは念頭に置きつつも)。word があるとわかった時、`used`をリセットしなくなるが、あとは return するだけだから気にしなくてもいい、のだろうか。何か自分が勘違いをしていた場合にやらかしそうでやや抵抗があるが。
