...
今回の制約から、 Trie の構築が最大 1000 * 20 = 2 * 10^4 回の処理 + DPがだいたい 300 * 20 = 6 * 10^3 で大雑把に合計で 3 * 10^4 回の処理が必要。C++が1秒間に10^8 (~ 10^9) 回の処理を行えるとすると、3 * 10^4 / 10^8 s = 300 us で Step 1 のだいたい 60 ms という見込みよりもかなり速くなりそう。(実装はPythonを用いてしまったが...)
