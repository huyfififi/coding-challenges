...
少し悩んだが、今回の場合先にチェックしても後にチェックしても code complexity がそこまで変わらないような気がするので、先にチェックする方法でstep2, 3に進もうと思う。

と思ったが、まだqueueにpushしただけの時にvisitedをtrueにするのは少し違和感があった。queueにいい名前が付けられれはいいのだが、全く思い浮かばなかったので、後からチェックする方法で実装しようと思う。
...
プリミティブ型のローカル変数には const を付けない

`static_cast<target-type ﻿>(expression ﻿)`

定数は static なメンバ変数か、グローバルに定義
