...
nが大きいとき、最後の項が支配的なので、総関数呼び出し数はO(n!)
各関数呼び出しで、`remaining[:i] + remaining[i + 1 :]`と`generating + [remaining[i]]`の操作により合計で長さnのオーダーのlist操作が発生するので時間計算量は O(n * n!)

空間計算量（補助空間）は関数呼び出しの深さが最大n、また各関数呼び出しにおいて`remaining`と`generating`の合計で長さnの新しいlistを作り保持するので、総じてO(n^2)
出力も含めると、長さnのpermutation n!通り全て保存するので O(n * n!) だが、出力ってspace complexityを出す時に入れるんだったっけ (-> space complexityを軽くGoogleしてみたけど、input/outputを区別せず、単にinputの大きさに対するメモリ使用量の度合いを指しているよう。)
